...
def trace_model_execution(model, reapply_on_copied_layers=False):
    ...
    used_as_input = []
    for x in outputs:
        used_as_input.append(x)
    tmp = []
    i = 1
    for l, Xs, Ys in reverse_executed_nodes:
        hey = []
        Ys = iutils.to_list(Ys)
        for y in Ys:
            for x in used_as_input:
                if x is y:
                    hey.append(True)
        if len(hey) >= len(Ys):
            i += 1
            Xs = iutils.to_list(Xs)
            used_as_input = used_as_input + Xs
            tmp.append((l, Xs, Ys))

    executed_nodes = list(reversed(tmp))
    inputs = model.inputs
    return layers, executed_nodes, outputs, inputs


def createATrace(executed_tensors, output):
    used_as_input = []
    used_as_input.append(output)
    xecuted_tensors1 = reversed(executed_tensors)
    tmp = []
    for l, Xs, Ys, visited in reversed(list(xecuted_tensors1)):
        if all([y in used_as_input for y in Ys]):
            used_as_input += Xs
            tmp.append((l, Xs, Ys))

    # executed_tensors = list(reversed(tmp))
...
def reverse_model(model, reverse_mappings,
    ...
    for _nid, (layer, Xs, Ys) in enumerate(reverse_execution_list):
        ...
        if isinstance(layer, keras.layers.InputLayer):
            ...
            modified_LRP_Xs = []
            if isinstance(reversed_Xs, tuple):
                print("hey" + str(_nid))
                modified_LRP_Xs = reversed_Xs[1]
                reversed_Xs = reversed_Xs[0]
            else:
                pass
